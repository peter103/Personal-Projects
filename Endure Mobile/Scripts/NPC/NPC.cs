...
namespace Endure
{
    ...
    public class NPC : MonoBehaviour
    {
        ...
        [Tooltip("Command Voice SFX")]
        public AudioClip[] commandVoiceSFX;
        [Tooltip("Calm Down Voice SFX")]
        public AudioClip[] calmDownVoiceSFX;

        private MusicPlayer music_player;
        private NPCCounter npc_counter;
        public GameObject npcParentNode;


        public int friendsLeft;

        public bool speakOnce = true;
        ...
        private void Start()
        {
            ...
            npc_counter = npcParentNode.GetComponent<NPCCounter>(); // Get the NPC Counter Class
        ...
        private void AttackPlayer()
        {
            ...
            {
                music_player.setFightMusic(true);
        ...
        // Play Alert audio file for other NPC to know.
        public void PlayVoiceCommand()
        {
            if(speakOnce)
            {
                source.PlayOneShot(commandVoiceSFX[Random.Range(0, commandVoiceSFX.Length)]);
            }
            
        }

        public void PlayVoiceCalmDown()
        {
            if (!speakOnce)
            {
                source.PlayOneShot(calmDownVoiceSFX[Random.Range(0, calmDownVoiceSFX.Length)]);
            }
        }
        ...
        public void Death()
        {
            npc_counter.npc_amount--;

            if(npc_counter.npc_amount <= 0)
            {
                npc_counter.setAllDead(true);
            }
